...
$(document).ready(function () {
    ...
    if (activeTab) {
        ...
        $('.tab-link[data-tab="checkout"]').addClass('active');
        $('#checkout').addClass('active-tab');
    }
...
async function getPendingCarts() {
    const token = localStorage.getItem('token');
    try {
        const response = await fetch('http://localhost/MiniProjectAPI/api/Cart/pending-carts', {
            method: 'GET', 
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
...
function setCheckoutForm(cartId,cartItemId,userId,firstName, lastName,driverLicense, contactInfo,clientAddress, selectedVehicleId,vehicleMake,vehicleModel,vehicleReg,mileage,startDate,endDate,dailyRate,totalCost,pickupAddress,dropoffAddress,extras,employeeName ) {
...
document.getElementById('rentalCheckoutForm').addEventListener('submit', async function(event) {
    event.preventDefault(); 
    ...
    if (!mileage || !fuelLevel || !vehicleConditionCheck || !vehicleCondition || !fuelLevel) {
...
//console.log(formattedPickUpDate);
    ...
    const rentalData = {
        ...
        mileageAtRental: parseInt(mileage,10), 
        fuelLevel: fuelLevel, 
        conditionCheck: vehicleConditionCheck, 
    ...
    try {
        const response = await fetch('http://localhost/MiniProjectAPI/api/Rental', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
            ...
            body: JSON.stringify({
                ...
                MileageAtRental: rentalData.mileageAtRental,  
                Fuellevel: rentalData.fuelLevel,  
                ConditionCheck: rentalData.conditionCheck,  
                ConditionNotes: rentalData.conditionNotes,  
        ...
        if (response.ok) {
            ...
            const updateResponse = await fetch(`http://localhost/MiniProjectAPI/api/Cart/items/${cartItemId}/checkout`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
        ...
        if (updateResponse.ok) {
            ...
            console.log('Cart item status updated:', updateResult.message);
        } else {
            const updateError = await updateResponse.json();
            alert('Error updating cart item status: ' + updateError.message);
        }
        const cartId = document.getElementById('cartId').value;

        const cartresponse = await fetch(`http://localhost/MiniProjectAPI/api/Cart/${cartId}/check-out-cart`, {
            method: 'POST',  
            headers: {
                'Content-Type': 'application/json',  
                'Authorization': `Bearer ${token}`  
        ...
        if (!cartresponse.ok) {
            ...
            return { success: false, message: errorData.message || 'Failed to check out cart.' };
        }
        const results = await cartresponse.json();
        console.log('Cart checked out successfully:', results);


            $('#checkoutModal').modal('hide'); 
            document.getElementById('rentalCheckoutForm').reset(); 
        } else {
